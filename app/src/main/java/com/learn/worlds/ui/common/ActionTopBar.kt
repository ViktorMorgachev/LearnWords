...
sealed class IconLeftAppBar{
    data class NavMenuIcon(val imageVector: ImageVector = Icons.Filled.Menu, val action: ()->Unit = { NavigationDrawerMediator.open() }): IconLeftAppBar()
    data class NavBackIcon(val imageVector: ImageVector = Icons.Filled.ArrowBack, val action: ()->Unit): IconLeftAppBar()
}
