...
class FirebaseStorageService @Inject constructor() {
    ...
    sealed class Path {
        data class Speech(val language: FirebaseStorageLanguage = FirebaseStorageLanguage.English, val name: String) : Path()
    ...
    fun getStorageRef(path: Path): StorageReference {
      val storageRef = when(path){
            ...
            is Path.Speech ->  storage.reference.child("speech_list/${path.language.desc}/${path.name}.mp3")
        }
        return storageRef
    }
}